...
def character_random_values(
    total_frams, difference, list_of_values, option, list_of_data
):

    option_data = random_value_genrator(
        total_frams, difference=difference, list_of_values=list_of_values
    )
...
def gentle_to_csv(gentle_json):
    ...
    list_of_data = character_random_values(
        total_frams, 7, [1, 2], "intensity", list_of_data
    )
    list_of_data = character_random_values(
        total_frams, 10, [1, 2, 3], "head_direction", list_of_data
    )
    list_of_data = character_random_values(
        total_frams, 5, [1, 2, 3], "eyes_direction", list_of_data
    )
    list_of_data = character_random_values(
        total_frams, 5, [False], "reaction_bool", list_of_data
    )
    list_of_data = character_random_values(
        total_frams, 5, [4], "screen_mode", list_of_data
    )
    list_of_data = character_random_values(
        total_frams, 7, [1, 2, 3, 4, 5, 6, 7, 8], "body_action", list_of_data
    )
    list_of_data = character_random_values(
        total_frams, 10, [1, 5, 6, 14], "emotion", list_of_data
    )
    list_of_data = character_random_values(
        total_frams, 40, [1.5, 0], "room", list_of_data
    )
    list_of_data = character_random_values(
        total_frams, 40, [9], "character", list_of_data
    )
