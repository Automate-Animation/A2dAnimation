...
for each_eye_key, value in df["eye_name"].items():
    ...
    if value != "nan":
        try:
            clean_eye_json[value] = {
                "eye_size": [int(i) for i in df["eye_size"][each_eye_key].split(",")],
                "eye_location": [
                    int(i) for i in df["eye_location"][each_eye_key].split(",")
                ],
            }
        except:
            pass
...
for each_eye_key, value in df["imouth_name"].items():
    print(value)
    if value != "nan":
        clean_mouth_json[value] = {
            "mouth_size": [int(i) for i in df["mouth_size"][each_eye_key].split(",")],
            "mouth_location": [
                int(i) for i in df["mouth_location"][each_eye_key].split(",")
            ],
        }
