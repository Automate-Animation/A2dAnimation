...
for each_file in files:
    ...
    if character_number not in character_body_cordination_data:
        ...
        character_body_cordination_data[character_number] = json.load(open(mypath + each_file))
...
def adding_background(body_path, background_path, frame_name, rotation=0, zoom=0):
    ...
    if character_body_cordination_data[character_number][screen_mode]["addition"]:
        ...
        new_image = adding_image(background, body_path,
                                location=character_body_cordination_data[character_number][screen_mode]["body_position"],
                                rotation=rotation,
                                mirror=character_body_cordination_data[character_number][screen_mode]["mirror"],
                                size_cordinates=character_body_cordination_data[character_number][screen_mode]["body_size"])
        
        #adding image after the main character
        if extra_image_data[character_number][screen_mode]['extra_after']:
            for each_extra_image in extra_image_data[character_number][screen_mode]['extra_after']:
                    new_image = adding_image(new_image, Image.open(each_extra_image['image']),
        ...
        if zoom > 0 and zoom < 8:
            ...
            x = character_body_cordination_data[character_number][screen_mode]["zoom_point"][0]
            y = character_body_cordination_data[character_number][screen_mode]["zoom_point"][1]
